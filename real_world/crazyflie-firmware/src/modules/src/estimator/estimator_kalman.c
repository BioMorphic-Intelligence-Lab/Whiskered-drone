...
// NO_DMA_CCM_SAFE_ZERO_INIT static kalmanCoreData_t coreData_wodynamics;
NO_DMA_CCM_SAFE_ZERO_INIT static kalmanCoreData_t coreData_wforce;
// NO_DMA_CCM_SAFE_ZERO_INIT static kalmanCoreData_t coreData_wodynamics_world;
NO_DMA_CCM_SAFE_ZERO_INIT static kalmanCoreData_t coreData_world;
/**
...
// static kalmanCoreParams_t coreParams_wodynamics;
static kalmanCoreParams_t coreParams_wforce;
...
static void updateQueuedMeasurements(const uint32_t nowMs, const bool quadIsFlying, bool whiskerTriggered);
...
void estimatorKalmanTaskInit() {
  ...
  // kalmanCoreDefaultParams(&coreParams_wodynamics);
  kalmanCoreDefaultParams(&coreParams_wforce);
...
static void kalmanTask(void* parameters) {
  ...
  logVarId_t idwhisker1 = logGetVarId("app", "PreprocessWhisker1_1");
  logVarId_t idwhisker2 = logGetVarId("app", "PreprocessWhisker2_1");
  logVarId_t iddepth1 = logGetVarId("app", "KFMlpOutput1");
  logVarId_t iddepth2 = logGetVarId("app", "KFMlpOutput2");
  while (true) {
    ...
    kalmanCoreDecoupleXY(&coreData_wodynamics);
    kalmanCoreDecoupleXY(&coreData_wforce);
  #endif

    // Run the system dynamics to predict the state forward.
    float whisker1 = logGetFloat(idwhisker1);
    float whisker2 = logGetFloat(idwhisker2);
    float depth1 = logGetFloat(iddepth1);
    float depth2 = logGetFloat(iddepth2);
    bool whiskerTriggered = (whisker1 > 20) || (whisker2 > 20);
    if (nowMs >= nextPredictionMs) {
      axis3fSubSamplerFinalize(&accSubSampler);
      axis3fSubSamplerFinalize(&gyroSubSampler);

      kalmanCorePredict(&coreData, &accSubSampler.subSample, &gyroSubSampler.subSample, nowMs, quadIsFlying, depth1, depth2, false, false);
      // kalmanCorePredict(&coreData_wodynamics, &accSubSampler.subSample, &gyroSubSampler.subSample,
      //             nowMs, quadIsFlying, depth1, depth2, false, false);
      kalmanCorePredict(&coreData_wforce, &accSubSampler.subSample, &gyroSubSampler.subSample,
                  nowMs, quadIsFlying, depth1, depth2, whiskerTriggered, true);
    ...
    kalmanCoreAddProcessNoise(&coreData, &coreParams, nowMs, false, false);
    // kalmanCoreAddProcessNoise(&coreData_wodynamics, &coreParams_wodynamics, nowMs, true, false);
    kalmanCoreAddProcessNoise(&coreData_wforce, &coreParams_wforce, nowMs, whiskerTriggered, true);
    updateQueuedMeasurements(nowMs, quadIsFlying, whiskerTriggered);

    if (kalmanCoreFinalize(&coreData_wforce))
    {
      STATS_CNT_RATE_EVENT(&finalizeCounter);
    }
    // kalmanCoreFinalize(&coreData_wodynamics);
    kalmanCoreFinalize(&coreData);
    if (! kalmanSupervisorIsStateWithinBounds(&coreData_wforce)) {
    ...
    kalmanCoreExternalizeState(&coreData_wforce, &taskEstimatorState, &accLatest);
    // kalmanCoreDroneToWorld(&coreData_wodynamics, &coreData_wodynamics_world);
    kalmanCoreDroneToWorld(&coreData, &coreData_world);
        ...
        switch (m.type) {
            case MeasurementTypeTDOA:
                if (robustTdoa) {
                    ...
                    // kalmanCoreRobustUpdateWithTdoa(&coreData_wodynamics, &m.data.tdoa, &outlierFilterTdoaState);
                    kalmanCoreRobustUpdateWithTdoa(&coreData_wforce, &m.data.tdoa, &outlierFilterTdoaState);
                } else {
                    kalmanCoreUpdateWithTdoa(&coreData, &m.data.tdoa, nowMs, &outlierFilterTdoaState);
                    // kalmanCoreUpdateWithTdoa(&coreData_wodynamics, &m.data.tdoa, nowMs, &outlierFilterTdoaState);
                    kalmanCoreUpdateWithTdoa(&coreData_wforce, &m.data.tdoa, nowMs, &outlierFilterTdoaState);
                }
                break;

            case MeasurementTypePosition:
                kalmanCoreUpdateWithPosition(&coreData, &m.data.position);
                // kalmanCoreUpdateWithPosition(&coreData_wodynamics, &m.data.position);
                kalmanCoreUpdateWithPosition(&coreData_wforce, &m.data.position);
                break;

            case MeasurementTypePose:
                kalmanCoreUpdateWithPose(&coreData, &m.data.pose);
                // kalmanCoreUpdateWithPose(&coreData_wodynamics, &m.data.pose);
                kalmanCoreUpdateWithPose(&coreData_wforce, &m.data.pose);
                break;

            case MeasurementTypeDistance:
                if (robustTwr) {
                    kalmanCoreRobustUpdateWithDistance(&coreData, &m.data.distance);
                    // kalmanCoreRobustUpdateWithDistance(&coreData_wodynamics, &m.data.distance);
                    kalmanCoreRobustUpdateWithDistance(&coreData_wforce, &m.data.distance);
                } else {
                    kalmanCoreUpdateWithDistance(&coreData, &m.data.distance);
                    // kalmanCoreUpdateWithDistance(&coreData_wodynamics, &m.data.distance);
                    kalmanCoreUpdateWithDistance(&coreData_wforce, &m.data.distance);
                }
                break;

            case MeasurementTypeTOF:
                kalmanCoreUpdateWithTof(&coreData, &m.data.tof);
                // kalmanCoreUpdateWithTof(&coreData_wodynamics, &m.data.tof);
                kalmanCoreUpdateWithTof(&coreData_wforce, &m.data.tof);
                break;

            case MeasurementTypeAbsoluteHeight:
                kalmanCoreUpdateWithAbsoluteHeight(&coreData, &m.data.height);
                // kalmanCoreUpdateWithAbsoluteHeight(&coreData_wodynamics, &m.data.height);
                kalmanCoreUpdateWithAbsoluteHeight(&coreData_wforce, &m.data.height);
                break;

            case MeasurementTypeFlow:
                kalmanCoreUpdateWithFlow(&coreData, &m.data.flow, &gyroLatest, false);
                // kalmanCoreUpdateWithFlow(&coreData_wodynamics, &m.data.flow, &gyroLatest);
                kalmanCoreUpdateWithFlow(&coreData_wforce, &m.data.flow, &gyroLatest, whiskerTriggered);
                break;

            case MeasurementTypeYawError:
                kalmanCoreUpdateWithYawError(&coreData, &m.data.yawError);
                // kalmanCoreUpdateWithYawError(&coreData_wodynamics, &m.data.yawError);
                kalmanCoreUpdateWithYawError(&coreData_wforce, &m.data.yawError);
                break;

            case MeasurementTypeSweepAngle:
                kalmanCoreUpdateWithSweepAngles(&coreData, &m.data.sweepAngle, nowMs, &sweepOutlierFilterState);
                // kalmanCoreUpdateWithSweepAngles(&coreData_wodynamics, &m.data.sweepAngle, nowMs, &sweepOutlierFilterState);
                kalmanCoreUpdateWithSweepAngles(&coreData_wforce, &m.data.sweepAngle, nowMs, &sweepOutlierFilterState);
                break;
            ...
            case MeasurementTypeBarometer:
                if (useBaroUpdate) {
                    ...
                    // kalmanCoreUpdateWithBaro(&coreData_wodynamics, &coreParams_wodynamics, m.data.barometer.baro.asl, quadIsFlying);
                    kalmanCoreUpdateWithBaro(&coreData_wforce, &coreParams_wforce, m.data.barometer.baro.asl, quadIsFlying);
                }
                break;
...
void estimatorKalmanInit(void)
{
  ...
  // kalmanCoreInit(&coreData_wodynamics, &coreParams_wodynamics, nowMs);
  kalmanCoreInit(&coreData_wforce, &coreParams_wforce, nowMs);
}
 ...
 */
 ...
 /**
 * @brief State position in the global frame x without dynamics

 */
  // LOG_ADD(LOG_FLOAT, stateX_wo, &coreData_wodynamics_world.S[KC_STATE_X])
 /**
 * @brief State position in the global frame y without dynamics

 */
  // LOG_ADD(LOG_FLOAT, stateY_wo, &coreData_wodynamics_world.S[KC_STATE_Y])
 /**
 * @brief State position in the global frame x with force

 */
  LOG_ADD(LOG_FLOAT, stateX_wof, &coreData_world.S[KC_STATE_X])
 /**
 * @brief State position in the global frame y with force

 */
  LOG_ADD(LOG_FLOAT, stateY_wof, &coreData_world.S[KC_STATE_Y])
 /**
 ...
 */
  ...
  */
  ...
  LOG_ADD(LOG_FLOAT, statePX_wof, &coreData_world.S[KC_STATE_PX])
  /**
  * @brief State velocity in its body frame y
  *
  *  Note: This should be part of stateEstimate
  */
  LOG_ADD(LOG_FLOAT, statePY_wof, &coreData_world.S[KC_STATE_PY])
  /**
  ...
  */
  ...
  /**
  * @brief Covariance matrix position x_wf
  */
  LOG_ADD(LOG_FLOAT, varX_wf, &coreData_wforce.P[KC_STATE_X][KC_STATE_X])
  /**
  * @brief Covariance matrix position y
  */
  LOG_ADD(LOG_FLOAT, varY_wf, &coreData_wforce.P[KC_STATE_Y][KC_STATE_Y])
  /**
  ...
  * @brief Covariance matrix velocity x
  */
  LOG_ADD(LOG_FLOAT, varPX_wf, &coreData_wforce.P[KC_STATE_PX][KC_STATE_PX])
  /**
  * @brief Covariance matrix velocity y
  */
  LOG_ADD(LOG_FLOAT, varPY_wf, &coreData_wforce.P[KC_STATE_PY][KC_STATE_PY])
  /**
