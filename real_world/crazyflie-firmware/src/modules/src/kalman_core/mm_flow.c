...
void kalmanCoreUpdateWithFlow(kalmanCoreData_t* this, const flowMeasurement_t *flow, const Axis3f *gyro, bool whiskerTriggered)
{
  ...
  //First update
  if (whiskerTriggered){
    kalmanCoreScalarUpdate(this, &Hx, (measuredNX-predictedNX), flow->stdDevX*FLOW_RESOLUTION*2.0f);
  } else{
  ...
  // Second update
  if (whiskerTriggered){
    kalmanCoreScalarUpdate(this, &Hy, (measuredNY-predictedNY), flow->stdDevY*FLOW_RESOLUTION*2.0f);
  } else{
