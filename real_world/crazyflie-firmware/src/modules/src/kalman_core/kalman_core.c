...
static const float MASS = 0.045f;
static float EI_CONST = 1e-4f; // E*I (N·m^2) - replace if you have better value
static float MU_FRICTION = 0.5f;
static const float WHISKER_LO = 0.2f;
static const float WHISKER_SPACING = 0.05f;
static const float WHISKER_PLACEMENT_RAD = M_PI_F/4;
// 使用一阶低通滤波器平滑外力变化
// static float ax_ext_filtered = 0.0f;
const float alpha = 0.2f; // 平滑系数，需要调试
float ax_ext_out = 0.0f, ay_ext_out = 0.0f, af_friction_out=0.0f;
...
float whisker_compute_N(float lo, float d, float alpha_rad)
{
    // contact point in sensor/body frame
    float Px = lo - d * cosf(alpha_rad);
    float Py = d * sinf(alpha_rad);

    // squared norm
    float Pnorm2 = Px*Px + Py*Py;
    if (Pnorm2 < EPS_force) return 0.0f;

    // theta (rotation angle)
    float theta = atan2f(Py, Px); // in radians

    // beta ~ 3/2 theta, n = tan(beta)
    float beta = 1.5f * theta;
    float n = tanf(beta);

    // gamma (characteristic radius factor)
    float gamma = 0.841655f - 0.0067807f * n + 0.000438f * n * n;

    // polynomial factor
    float poly = COEF_A - COEF_B * n + COEF_C * n * n
                  - COEF_D * n * n * n + COEF_EPOLY * n * n * n * n;

    // denominator terms
    float denom_ang = cosf(theta) + n * sinf(theta);
    float denom_geom = sinf(beta + alpha_rad); // sin(3/2 theta + alpha)

    // protect against near-zero denominators (model singularities)
    if (fabsf(denom_ang) < EPS_force) return 0.0f;
    if (fabsf(denom_geom) < EPS_force) return 0.0f;

    // bending force (F_bend)
    float F_bend = (EI_CONST * poly * theta * gamma * gamma) / (Pnorm2 * denom_ang);

    // normal-to-plane force N
    float N = F_bend / denom_geom;
 ...
 */
void whisker_force_vector_2d(float lo,
                                    float d_left, float d_right,
                                    float spacing, float placement_rad,
                                    float *outFx, float *outFy)
{
    // 1. 两个 whisker 的 bending force（沿 x 轴的大小）
    float N_left  = whisker_compute_N(lo, d_left, placement_rad);
    float N_right = whisker_compute_N(lo, d_right, placement_rad);
    ...
    // 2. 用深度差和间距算出墙壁斜率
    float wall_vector = (d_left - d_right) / spacing;
...
static void predictDt(kalmanCoreData_t* this, Axis3f *acc, Axis3f *gyro, float dt, bool quadIsFlying, float d_left, float d_right, bool whiskerTriggered, bool useForce)
{
  ...
  if (quadIsFlying) // only acceleration in z direction
  {
    ...
    dx = this->S[KC_STATE_PX] * dt + ax_ext_out * dt2 / 2.0f;  //add acc?
    dy = this->S[KC_STATE_PY] * dt + (ay_ext_out + af_friction_out) * dt2 / 2.0f;  //add acc?
    ...
    if (useForce) {
        float ax_ext = 0.0f;
        float ay_ext = 0.0f;
        float af_friction = 0.0f;
        float fx = 0.0f, fy = 0.0f, f_friction = 0.0f;
        if (whiskerTriggered) {
          // float fx = 0.0, fy = 0.0;
          whisker_force_vector_2d(WHISKER_LO, d_left / 1000.0f, d_right / 1000.0f, WHISKER_SPACING, WHISKER_PLACEMENT_RAD, &fx, &fy);

          // 转换为加速度
          ax_ext = fx/MASS;
          ay_ext = 0.0f;
          if (tmpSPY < 0.0f) {
              f_friction = -copysignf(1.0f, tmpSPY) * MU_FRICTION * fabsf(fx);
              af_friction = f_friction / MASS;
          } else {
              f_friction = 0.0f;
              af_friction = 0.0f;
          }
        ...
        // 平滑过渡
        // ax_ext_filtered = alpha * ax_ext + (1 - alpha) * ax_ext_filtered;
        ax_ext_out = alpha * ax_ext + (1 - alpha) * ax_ext_out;
        ay_ext_out = 0.0f ;
        af_friction_out =alpha * af_friction + (1 - alpha) * af_friction_out ;
        this->S[KC_STATE_PX] += dt * (gyro->z * tmpSPY - gyro->y * tmpSPZ 
                                    - GRAVITY_MAGNITUDE * this->R[2][0] + ax_ext); 
        this->S[KC_STATE_PY] += dt * (-gyro->z * tmpSPX + gyro->x * tmpSPZ 
                                    - GRAVITY_MAGNITUDE * this->R[2][1] + ay_ext + af_friction);
        this->S[KC_STATE_PZ] += dt * (zacc + gyro->y * tmpSPX - gyro->x * tmpSPY 
...
void kalmanCorePredict(kalmanCoreData_t* this, Axis3f *acc, Axis3f *gyro, const uint32_t nowMs, bool quadIsFlying, float d_left, float d_right, bool whiskerTriggered, bool useForce) {
  float dt = (nowMs - this->lastPredictionMs) / 1000.0f;
  predictDt(this, acc, gyro, dt, quadIsFlying, d_left, d_right, whiskerTriggered, useForce);
  this->lastPredictionMs = nowMs;
}


static void addProcessNoiseDt(kalmanCoreData_t *this, const kalmanCoreParams_t *params, float dt, bool whiskerTriggered, bool useForce)
{
  ...
  // if whisker trigger, add noise
  if (whiskerTriggered && !useForce) {
    // const float extraNoise = 1000.0f;  // dynamic model noise
    this->P[KC_STATE_X][KC_STATE_X] = 0;
    this->P[KC_STATE_Y][KC_STATE_Y] = 0;
    this->P[KC_STATE_PX][KC_STATE_PX] = 0;
    this->P[KC_STATE_PY][KC_STATE_PY] = 0;
  }
  else if (whiskerTriggered && useForce) {
      // 原本增加的过程噪声
      // const float extraNoise = powf(params->procNoiseAcc_xy*dt*dt + params->procNoiseVel*dt + params->procNoisePos, 2);
      // 位置
      this->P[KC_STATE_X][KC_STATE_X] += powf(params->procNoiseAcc_ext*dt*dt, 2);
      this->P[KC_STATE_Y][KC_STATE_Y] += powf(params->procNoiseAcc_ext*dt*dt, 2);

      // 速度
      this->P[KC_STATE_PX][KC_STATE_PX] += powf(params->procNoiseAcc_ext*dt, 2);
      this->P[KC_STATE_PY][KC_STATE_PY] += powf(params->procNoiseAcc_ext*dt, 2);
  }
  for (int i=0; i<KC_STATE_DIM; i++) {
    for (int j=i; j<KC_STATE_DIM; j++) {
      float p = 0.5f*this->P[i][j] + 0.5f*this->P[j][i];
...
void kalmanCoreAddProcessNoise(kalmanCoreData_t *this, const kalmanCoreParams_t *params, const uint32_t nowMs, bool whiskerTriggered, bool useForce) {
  float dt = (nowMs - this->lastProcessNoiseUpdateMs) / 1000.0f;
  if (dt > 0.0f) {
    addProcessNoiseDt(this, params, dt, whiskerTriggered, useForce);
    this->lastProcessNoiseUpdateMs = nowMs;
  }
}
...
void kalmanCoreDroneToWorld(const kalmanCoreData_t* coreData_body,
                                  kalmanCoreData_t* coreData_world)
{
    // --- 位置在 body/world 系统一样，直接拷贝 ---
    coreData_world->S[KC_STATE_X] = coreData_body->S[KC_STATE_X];
    coreData_world->S[KC_STATE_Y] = coreData_body->S[KC_STATE_Y];
    coreData_world->S[KC_STATE_Z] = coreData_body->S[KC_STATE_Z];

    // --- 速度从 body frame 旋转到 world frame ---
    coreData_world->S[KC_STATE_PX] =
        coreData_body->R[0][0]*coreData_body->S[KC_STATE_PX] +
        coreData_body->R[0][1]*coreData_body->S[KC_STATE_PY] +
        coreData_body->R[0][2]*coreData_body->S[KC_STATE_PZ];

    coreData_world->S[KC_STATE_PY] =
        coreData_body->R[1][0]*coreData_body->S[KC_STATE_PX] +
        coreData_body->R[1][1]*coreData_body->S[KC_STATE_PY] +
        coreData_body->R[1][2]*coreData_body->S[KC_STATE_PZ];

    coreData_world->S[KC_STATE_PZ] =
        coreData_body->R[2][0]*coreData_body->S[KC_STATE_PX] +
        coreData_body->R[2][1]*coreData_body->S[KC_STATE_PY] +
        coreData_body->R[2][2]*coreData_body->S[KC_STATE_PZ];
}
 ...
 */
  LOG_ADD(LOG_FLOAT, fx, &ax_ext_out)
 /**
 * @brief State position in the global frame y
 *
 *  Note: This is similar to stateEstimate.y
 */
  LOG_ADD(LOG_FLOAT, fy, &ay_ext_out)

  LOG_ADD(LOG_FLOAT, friction, &af_friction_out)

LOG_GROUP_STOP(kalman)

PARAM_GROUP_START(kalman)
PARAM_ADD(PARAM_FLOAT, MU_FRICTION, &MU_FRICTION)
PARAM_ADD(PARAM_FLOAT, EI_CONST, &EI_CONST)
