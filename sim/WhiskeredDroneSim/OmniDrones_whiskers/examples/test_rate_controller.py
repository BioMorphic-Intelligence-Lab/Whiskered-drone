...
def main(cfg):
    ...
    for i in tqdm(range(300)):
        ...
        # 获取 state（13 是 pos+quat+vel）
        state13 = drone.get_state(check_nan=False, env_frame=True)[..., :13]  # shape: (1?,13)
        pos = state13[..., 0:3]
        quat = state13[..., 3:7]
        vel_lin = state13[..., 7:10]
        body_rates = state13[..., 10:13]  # p,q,r

        # 若你在类里维护了 vel_w / vel_b，可以直接打印它们（优先 body 角速度）
        # 注意：确保属性存在，否则做个保护
        vel_w = getattr(drone, "vel_w", None)
        vel_b = getattr(drone, "vel_b", None)

        # 把四元数转成欧拉（方便观察 yaw/pitch/roll）
        euler = quaternion_to_euler(quat)  # 返回 (roll, pitch, yaw)

        # 计算误差与控制输出
        rate_error = (target_rate - body_rates).detach().cpu().numpy()
        height = drone.pos[..., 2].cpu().numpy()
        height_error = (target_height.cpu().numpy() - height)
        # action 里具体结构依赖 controller（假设 action 含 thrust 或者 4 个电机值）
        action_np = action.detach().cpu().numpy()

        print(f"Step {i}:")
        print(f"  pos (x,y,z): {pos.detach().cpu().numpy()}")
        print(f"  euler (r,p,y): {euler.detach().cpu().numpy()}")
        print(f"  lin vel (x,y,z): {vel_lin.detach().cpu().numpy()}")
        print(f"  body rates (p,q,r): {body_rates.detach().cpu().numpy()}")
        print(f"  target_rate: {target_rate.detach().cpu().numpy()}")
        print(f"  rate_error: {rate_error}")
        if vel_w is not None:
            print(f"  vel_w (first env): {vel_w.detach().cpu().numpy()[:3]}")
        if vel_b is not None:
            print(f"  vel_b (first env): {vel_b.detach().cpu().numpy()[:3]}")
        print(f"  height: {height}  height_err: {height_error}")
        print(f"  action (controller output): {action_np}")
        print("-" * 60)
